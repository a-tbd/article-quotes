...
def get_quotes():
    words_to_match = set(['said', 'asked', 'added', 'noted', 'replied', 'responded',
                          'wrote', 'emailed', 'stated', 'accused', 'shouted', 'demanded'])
    ...
    # some ppl are missing from the lookup table
    all_people = {n.root: n for n in n_chunks if n.root.ent_type_ == 'PERSON'}
    ...
    for possible_verb in doc:
        if possible_verb.lower_ in words_to_match:
            ...
            possible_subs = [
                c for c in possible_verb.subtree if c.dep == nsubj or c.dep == npadvmod]

            for c in possible_subs:
                speaker = None
                if c.tag_ == 'NNP':
                    try:
                        # this is only looking up noun chunks
                        speaker = all_people[c]
                    except:
                        # http://www.goodmami.org/2013/01/30/Getting-only-the-first-match-in-a-list-comprehension.html
                        start = next(
                            (child.i for child in c.lefts if child.ent_type_ == 'PERSON'), c.i)
                        # TODO does this work for Mr.?
                        end = next(
                            (child.i for child in c.rights if child.ent_type_ == 'PERSON'), c.i)
                ...
                if speaker:
                    ...
                    speakers_sing.append(
                        (speaker, gender, doc[possible_verb.left_edge.i:possible_verb.right_edge.i+1]))
